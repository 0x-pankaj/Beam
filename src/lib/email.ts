...
const button = (href: string, label: string) =>
  `<a href="${escapeHtml(href)}" style="display:inline-block;background:#7c5cff;color:#fff;padding:12px 18px;border-radius:10px;text-decoration:none;font-weight:600">${escapeHtml(label)}</a>`;
...
export async function notifyCreatorPaid(opts: {
  ...
  const html = `
    <div style="font-family:system-ui,sans-serif;max-width:480px;margin:auto">
      ...
      <p>${opts.fromLabel ? `${escapeHtml(opts.fromLabel)} paid you ` : "You received "}${amount}${
        opts.what ? ` for <b>${escapeHtml(opts.what)}</b>` : ""
...
/** "🎉 You received money" — notify a walletless recipient their claim paid out. */
export async function notifyRecipientPaid(opts: {
  to?: string;
  amountUsd: string;
  fromLabel?: string;
}): Promise<void> {
  if (!opts.to) return;
  const amount = `$${Number(opts.amountUsd).toLocaleString()}`;
  const subject = `🎉 You received ${amount} on Beam`;
  const html = `
    <div style="font-family:system-ui,sans-serif;max-width:480px;margin:auto">
      <h2>${escapeHtml(subject)}</h2>
      <p>${opts.fromLabel ? `${escapeHtml(opts.fromLabel)} sent you ` : "You received "}${amount} — it settled to your account on Arbitrum. No wallet, no seed phrase.</p>
      <p>${button(APP_URL, "Open Beam")}</p>
    </div>`;
  await sendEmail({ to: opts.to, subject, html });
}
